...
import sys
sys.path.append('C:/Users/admin/Desktop/functions/')
...
df_ori = pd.read_excel('C:/Users/admin/Desktop/functions/gas_2015-2017.xlsx')
df = df_ori.copy()
...
if 0 == index:
    ...
    for i in range(len(df_train)-1, -1, -1):  # 使循环变量i按降序取值
        ...
        m.append(Prophet(holidays=holidays_custom, holidays_prior_scale=100,
                         ...
                         seasonality_mode='multiplicative', mcmc_samples=0, uncertainty_samples=100))
    end_1 = time.process_time()
else:
    choice = choice[index]
    start_1 = time.process_time()
...
for i in range(len(df_train)-1, -1, -1):  # 此for循环不能和前一个for循环合并，否则因为i按降序取值，则m[i]会超出索引范围
    ...
    m[i].add_seasonality(name='weekly_off_sf', period=7, fourier_order=5, prior_scale=500, condition_name='off_season',
                      mode='multiplicative')
    # m[i].add_seasonality(name='monthly', period=30.5, fourier_order=7, prior_scale=10, mode='multiplicative')
    # m[i].add_seasonality(name='seasonly', period=91, fourier_order=8, prior_scale=20, mode='multiplicative')
...
'''
 回测结果的MAPE为百分之 3.13 （此数值越接近0越好）
 回测结果的SMAPE为百分之 3.12 （此数值越接近0越好）
 回测结果的RMSE为 0.0507 （此数值越接近0越好）
 该算法回测结果与单步Naive回测结果的综合准确度指标之比为 0.91 （该数值越小越好）
 该算法回测结果与单步Naive回测结果的综合评估指标之比为 0.93 （该数值越小越好）
 使用 isin 方法生成所有回测序列的自定义列时，CPU运行时间为 3.250 秒 
 所有回测序列做拟合和预测时，CPU运行时间为 536.562 秒 
 程序运行总时间为 11.099 分
