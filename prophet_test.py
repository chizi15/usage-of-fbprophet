...
# 回测pred个点，回测频率为freq个点/次
pred, freq = 1000, 14
n = list(np.arange(freq, pred+1, freq))
df_train, df_test = [], []
for i, j in zip(n, range(len(n))):
    df_train.append(df[:-i])
    df_test.append(pd.DataFrame(pd.Series(df['ds'][len(df_train[j]): len(df_train[j])+freq]), columns=['ds']))

# 表示节前4天，节后9天，节日当天1天；则认为此数据集的春节效应期一共有14天，为周周期7天的倍数
lower_window = -4
upper_window = 9
...
if 0 == index:
    ...
    def is_sf_season(ds, date_ranges):
        ...
        if sum(ds == date_ranges) == 1:
            ...
            return False
    for i in range(len(df_train) - 1, -1, -1):  # 使循环变量i按降序取值
        # 此循环中，apply要做 len(df_train[i]['ds']) × len(date_ranges) × len(df_train[i]) 次判断，速度较慢；改用pd.merge生成自定义列
...
MAPE = ref.mape(y_true=comp['y_true'], y_pred=comp['yhat'])
SMAPE = ref.smape(y_true=comp['y_true'], y_pred=comp['yhat'])
RMSE = ref.rmse(y_true=comp['y_true'], y_pred=comp['yhat'])
naive = df['y'][-len(comp['y_true'])-1:-1].reset_index(drop=True)
...
print(' 该算法回测结果与单步Naive回测结果的综合准确度指标之比为 %.2f （该数值小于1且越接近0越好）' % (accuracy[0][0] / accuracy[0][1]))
print(' 该算法回测结果与单步Naive回测结果的综合评估指标之比为 %.2f （该数值小于1且越接近0越好）' % (evaluation[0][0] / evaluation[0][1]))
